...
describe("canonical json + hashing + uuid", () => {
  ...
  it("separates request hashes by operation and schema version", () => {
    ...
    expect(requestHash("op", 1, { a: 1, b: 2 })).toBe(requestHash("op", 1, { b: 2, a: 1 }));
  });
...
describe("create-session idempotency", () => {
  ...
  it("rejects non-UUID keys without persisting", async () => {
    ...
    try {
      ...
      const n = (db.prepare("SELECT COUNT(*) AS n FROM api_idempotency").get() as { n: number }).n;
      expect(n).toBe(0);
    } finally {
      closeKernelDatabase(handle);
    }
...
describe("message idempotency + active uniqueness", () => {
  it("replays exact status/body and conflicts on different hash", async () => {
    const { handle, repo, db } = await openRepo();
    try {
      const s = repo.createSession({ key: messageKey(), now: T0 }).body.sessionId;
      const key = messageKey();
      const first = repo.postMessage(s, { text: "hello" }, { key, now: T0 });
      expect(first.status).toBe(202);
      const replay = repo.postMessage(s, { text: "hello" }, { key, now: T0 + 1 });
      expect(replay.replayed).toBe(true);
      expect(replay.body).toEqual(first.body);
      expect(replay.status).toBe(202);
      expect(() => repo.postMessage(s, { text: "different" }, { key, now: T0 })).toThrow(
        IdempotencyConflictError,
      );
      const turns = (db.prepare("SELECT COUNT(*) AS n FROM turns").get() as { n: number }).n;
      expect(turns).toBe(1);
    } finally {
      closeKernelDatabase(handle);
    }
  ...
  it("does not persist validation failures or session_busy", async () => {
    ...
    try {
      ...
      expect(count("SELECT COUNT(*) AS n FROM api_idempotency WHERE scope LIKE 'session:%'")).toBe(0);
      ...
      expect(count("SELECT COUNT(*) AS n FROM api_idempotency WHERE scope LIKE 'session:%'")).toBe(1);
    } finally {
      closeKernelDatabase(handle);
    }
  });
  ...
  it("isolates active runs across sessions and 404s unknown sessions", async () => {
    const { handle, repo } = await openRepo();
    try {
      const a = repo.createSession({ key: messageKey(), now: T0 }).body.sessionId;
      const b = repo.createSession({ key: messageKey(), now: T0 }).body.sessionId;
      repo.postMessage(a, { text: "a" }, { key: messageKey(), now: T0 });
      expect(() =>
        repo.postMessage(b, { text: "b" }, { key: messageKey(), now: T0 }),
      ).not.toThrow();
      expect(() =>
        repo.postMessage(generateId(), { text: "x" }, { key: messageKey(), now: T0 }),
...
describe("retry + allocators", () => {
  it("allocates seq/attempt without MAX and reuses immutable input", async () => {
    ...
    try {
      const s = repo.createSession({ key: messageKey(), now: T0 }).body.sessionId;
      const first = repo.postMessage(s, { text: "q" }, { key: messageKey(), now: T0 });
      ...
      const retry = repo.postRetry(s, turnId, { key: messageKey(), now: T0 + 3 });
      ...
      const pos = (db.prepare("SELECT next_turn_position AS p FROM sessions WHERE id = ?").get(s) as { p: number }).p;
      expect(pos).toBe(2);
    } finally {
      closeKernelDatabase(handle);
    }
  ...
  it("replays retry on same key and rejects cross-session turns", async () => {
    ...
    try {
      const s = repo.createSession({ key: messageKey(), now: T0 }).body.sessionId;
      const other = repo.createSession({ key: messageKey(), now: T0 }).body.sessionId;
      const m = repo.postMessage(s, { text: "q" }, { key: messageKey(), now: T0 });
      ...
      expect(() => repo.postRetry(other, m.body.turnId, { key: messageKey(), now: T0 })).toThrow(
        RepositoryNotFoundError,
      );
      expect(() => repo.postRetry(s, generateId(), { key: messageKey(), now: T0 })).toThrow(
        RepositoryNotFoundError,
      );
    } finally {
      closeKernelDatabase(handle);
...
describe("transitions + event atomicity", () => {
  it("starts queued runs once and appends run.started atomically", async () => {
    ...
    try {
      ...
      const n = (db.prepare("SELECT COUNT(*) AS n FROM run_events WHERE run_id = ?").get(m.body.run.id) as { n: number }).n;
      expect(n).toBe(2);
    } finally {
      closeKernelDatabase(handle);
    }
  ...
  it("completes only from running and upserts selection in the same commit", async () => {
    ...
    try {
      ...
      const early = repo.completeRun(m.body.run.id, { version: 1, text: "ans" }, { now: T0 + 1 });
      expect(early.applied).toBe(false);
      expect(repo.getSelection(m.body.turnId)).toBeNull();
      await repo.startRun(m.body.run.id, { now: T0 + 2 });
      const done = repo.completeRun(m.body.run.id, { version: 1, text: "ans" }, { now: T0 + 3 });
      ...
      const late = repo.completeRun(m.body.run.id, { version: 1, text: "other" }, { now: T0 + 4 });
      ...
      expect(types.map((t) => t.type)).toEqual(["run.queued", "run.started", "run.completed"]);
    } finally {
      closeKernelDatabase(handle);
    }
  });
  ...
  it("fails only from running without touching selection and rejects raw errors", async () => {
    ...
    try {
      ...
      expect(() => repo.failRun(m.body.run.id, "Raw error with details!!", { now: T0 })).toThrow(
        RepositoryValidationError,
      );
      const failed = repo.failRun(m.body.run.id, "execution_failed", { now: T0 + 2 });
  ...
  it("refuses event append after a terminal event", async () => {
    ...
    try {
      ...
      const m = repo.postMessage(s, { text: "q" }, { key: messageKey(), now: T0 });
      await repo.startRun(m.body.run.id, { now: T0 + 1 });
      await repo.completeRun(m.body.run.id, { version: 1, text: "ans" }, { now: T0 + 2 });
      expect(() =>
...
describe("cancel-first semantics", () => {
  ...
  it("discards completion after cancel_requested and finalizes via watchdog", async () => {
    ...
    try {
      ...
      const attempt = repo.completeRun(m.body.run.id, { version: 1, text: "late" }, { now: T0 + 3 });
  ...
  it("recover keeps queued and records restart_recovery cause", async () => {
    ...
    try {
      ...
      const payload = handle.raw
        .prepare("SELECT payload FROM run_events WHERE run_id = ? AND type = 'run.abandoned'")
        .get(running[0]?.id) as { payload: string };
      expect(JSON.parse(payload.payload)).toEqual({ cause: "restart_recovery" });
...
describe("selection rules", () => {
  it("failed/cancelled/abandoned never select; select_on_success=false keeps prior", async () => {
    ...
    try {
      const s = repo.createSession({ key: messageKey(), now: T0 }).body.sessionId;
      const m1 = repo.postMessage(s, { text: "q1" }, { key: messageKey(), now: T0 });
      ...
      const r1 = repo.postRetry(s, m1.body.turnId, { key: messageKey(), now: T0 + 3 });
      await repo.startRun(r1.body.run.id, { now: T0 + 4 });
      await repo.completeRun(r1.body.run.id, { version: 1, text: "first" }, { now: T0 + 5 });
      expect(repo.getSelection(m1.body.turnId)?.runId).toBe(r1.body.run.id);

      const r2 = repo.postRetry(
        s,
        m1.body.turnId,
        { key: messageKey(), now: T0 + 6, selectOnSuccess: false },
      );
      await repo.startRun(r2.body.run.id, { now: T0 + 7 });
      await repo.completeRun(r2.body.run.id, { version: 1, text: "second" }, { now: T0 + 8 });
      expect(repo.getSelection(m1.body.turnId)?.runId).toBe(r1.body.run.id);
    } finally {
      closeKernelDatabase(handle);
    }
...
describe("history + events pagination + idempotency lookup", () => {
  it("projects history with exclusive paging and hides unselected runs", async () => {
    ...
    try {
      const s = repo.createSession({ key: messageKey(), now: T0 }).body.sessionId;
      const m1 = repo.postMessage(s, { text: "first" }, { key: messageKey(), now: T0 });
      await repo.startRun(m1.body.run.id, { now: T0 + 1 });
      await repo.completeRun(m1.body.run.id, { version: 1, text: "ans1" }, { now: T0 + 2 });
      const m2 = repo.postMessage(s, { text: "second" }, { key: messageKey(), now: T0 + 3 });
      ...
      const page2 = repo.getHistory(s, { beforePosition: page1.nextBefore ?? 1, limit: 10 });
  ...
  it("pages events exclusively with terminal derived from status", async () => {
    ...
    try {
      ...
      const p2 = repo.getEvents(s, m.body.run.id, { after: p1.nextAfter, limit: 10 });
      expect(p2.events.map((e) => e.seq)).toEqual([2]);
      expect(p2.nextAfter).toBe(2);
      const empty = repo.getEvents(s, m.body.run.id, { after: p2.nextAfter, limit: 10 });
      ...
      expect(() => repo.getEvents(generateId(), m.body.run.id, { after: 0 })).toThrow(
        RepositoryNotFoundError,
      );
    } finally {
      closeKernelDatabase(handle);
  ...
  it("looks up idempotency scoped to the session", async () => {
    ...
    try {
      ...
      const missing = repo.lookupIdempotencyForSession(s, messageKey(), messageScope(s));
      expect(missing).toEqual({ found: false, code: "resend_required" });
      const other = repo.createSession({ key: messageKey(), now: T0 }).body.sessionId;
      expect(() => repo.lookupIdempotencyForSession(other, key, messageScope(s))).toThrow(
        RepositoryNotFoundError,
      );
    } finally {
      closeKernelDatabase(handle);
