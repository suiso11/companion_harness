...
> ラベル: **合意（agreed）**。M3（Conversation UI + SSE）の実装計画として合意した内容を記録する。合意事項（可視コントロール・SSE/履歴/Retry/Stop の exact 契約・リデューサ・リカバリ・セキュリティ方針）は実装時の調整対象ではない。調整対象は esbuild チューニング等の実装レベルのみ（§20）である。**実装状況（2026-09-08 検証時点・truthful / 未完 — M3 完了ではない）:** SSR シェル・SSE ルート配線（`apps/server/src/app.ts`）・`sse.ts`・UI（`src/ui/page.ts・reducer.ts・escape.ts・client.ts・client.css`）・esbuild バンドル（`scripts/build-ui.mjs`）・`test/m3_ui.test.ts`（13 件）に加え、Playwright smoke 4 件（`apps/server/e2e/conversation.spec.ts`: echo / type-while-active+stop-cancel / fail-then-retry / strict-CSP+minimal-localStorage）・harness（`e2e/server.ts`・`e2e/ports.ts`）・`playwright.config.ts`・専用 `apps/server/tsconfig.e2e.json`＋root `typecheck:e2e`・Ubuntu E2E CI ジョブ（frozen install・`playwright install --with-deps chromium`・E2E typecheck・production `build:ui`・smoke 実行・failure 成果物のみ）まで作業ツリーに実装済み。検証: root＋`apps/server` の `tsc --noEmit` は `e2e/`・config を含まないため harness 未カバー（E2E は専用 tsconfig/CI で検証）。Playwright 実行数は当環境では **0**（`pnpm install` が exit `3221226505` で crash し `@playwright/test` 未解決のためブラウザ未駆動）。未完了の受入ギャップ: 引用ドロワー/CAS・replay/reload 回復・SSE 再接続（重複 seq・gap catch-up・破損カーソル停止・JSON フォールバック）・旧履歴 pagination は smoke 対象外。本節の契約が実装基準であることに変わりはない。
