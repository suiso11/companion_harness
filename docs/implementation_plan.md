...
> 注記（合意の変更なし）: 上記「ベクトル DB / RAG / 長期記憶の自動構築」は **v0.1（M0〜M4 読み取り専用）および v0.2（M5）の範囲では非目標のまま** とする。v0.2 以降の外部長期記憶の検討は別文書 **[長期記憶計画](long_term_memory_plan.md)**（planning-only）にのみ置き、本書の合意を書き換えない。
...
> 計画名（Companion Harness / companion_harness）は確定済みである。本一覧は #1〜#5 の実装 spike のみを含む。長期記憶に関する未解決事項は本書では扱わず、別文書 **[長期記憶計画](long_term_memory_plan.md#12-ロードマップと未解決事項)** にのみ置く。いずれも合意ではない。
...
> 長期記憶の配置・承認・保持・評価に関する未解決事項は本書では番号を持たず、別文書 **[長期記憶計画](long_term_memory_plan.md#12-ロードマップと未解決事項)** にのみ置く。
...
> 本節は詳細計画を持たない。外部長期記憶の検討は別文書 **[長期記憶計画](long_term_memory_plan.md)**（planning-only）にのみ置く。本書は計画のみであり、実装・テスト・リリースはまだ行われていない。
>
> - **ユーザ決定（別リポジトリで実装）:** 外部長期記憶は Companion Harness とは **別のリポジトリで実装する**。リポジトリ名・パス・ランタイム・連携プロトコルは未定（TBD）であり、本書では定めない。
> - **M0〜M5・v0.1 読み取り専用・v0.2 承認フローの合意（§8・§9・§14〜§19）は一切変更しない。** Companion 側への記憶書き込みは、将来承認された連携契約に基づく場合にのみ可能であり、[§18](<#18-m5-action-plan合意>) の承認フロー（対象はカレンダーの start / end のみ）をバイパスしない。
> - 詳細設計と未解決事項は別文書の [§12 ロードマップと未解決事項](long_term_memory_plan.md#12-ロードマップと未解決事項) にのみ置き、本書では番号を持たない。
