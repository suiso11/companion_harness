...
> ラベル: **合意（agreed）**。M3（Conversation UI + SSE）の実装計画として合意した内容を記録する。合意事項（可視コントロール・SSE/履歴/Retry/Stop の exact 契約・リデューサ・リカバリ・セキュリティ方針）は実装時の調整対象ではない。調整対象は esbuild チューニング等の実装レベルのみ（§20）である。**実装状況（2026-09-08 検証時点・truthful / 未完 — M3 完了ではない）:** SSR シェル・SSE ルート配線（`apps/server/src/app.ts`）・`sse.ts`・UI（`src/ui/page.ts・reducer.ts・escape.ts・client.ts・client.css`）・esbuild バンドル（`scripts/build-ui.mjs`）・`test/m3_ui.test.ts`（13 件）に加え、Playwright 7 件のソース（`apps/server/e2e/conversation.spec.ts` 4 件: echo / type-while-active+stop-cancel / fail-then-retry / strict-CSP+minimal-localStorage ＋ `apps/server/e2e/citation_recovery.spec.ts` 3 件: escaped drawer+CAS-select/stale-PUT-409 / lost-delivery same-key replay+reload 回復 / unknown-pending-key resend-required）・harness（`e2e/server.ts`・`e2e/ports.ts`）・`playwright.config.ts`（`testMatch` は両 spec）・専用 `apps/server/tsconfig.e2e.json`＋root `typecheck:e2e`・Ubuntu E2E CI ジョブ（frozen install・`playwright install --with-deps chromium`・E2E typecheck・production `build:ui`・E2E 実行・failure 成果物のみ）まで作業ツリーに実装済み。さらに `apps/server/e2e/sse_history.spec.ts` 5 件のソース（native SSE reconnect / dropped-frame JSON gap catch-up / corrupt cursor ahead+malformed / seeded older-history）・SSE fault proxy（`e2e/server.ts` 内）・`playwright.config.ts` の `testMatch` 3 spec まで拡張済み（ソース計 12 件）。検証: 公開 CI run `34216072834`（HEAD `88097cb`）は旧 7 件時点の **completed success**であり、新 5 件の実行結果は未観測（ソース件数と実行件数の区別 — `gh` 未認証のため公開 REST/logs での件数検証不可）。HEAD `ecad20b` で ahead-cursor resync 後の live-run 再 subscribe（`client.ts`）＋ poison 正規化アサート（spec）を修正し push 済み。root＋`apps/server` の `tsc --noEmit` は `e2e/`・config を含まないため harness 未カバー（E2E は専用 tsconfig/CI で検証するもので、同等ではない）。Playwright 実行数は当環境では **0**（`@playwright/test` 未解決のためブラウザ未駆動・新規 install 禁止）。残りの受入ギャップ: 新 5 件の公開 CI 実行確認（run `34219889304` は pending のまま未観測）。本節の契約が実装基準であることに変わりはない。
